...
@router.get('/quota')
def get_quota(_: str = Depends(get_current_token)):
    return ok(service.get_quota().model_dump())


@router.get('/chat/session/today')
def get_today_session(_: str = Depends(get_current_token)):
    return ok([item.model_dump() for item in service.get_today_session()])


@router.post('/chat/ask')
def ask_ai(payload: AskAiRequest, _: str = Depends(get_current_token)):
