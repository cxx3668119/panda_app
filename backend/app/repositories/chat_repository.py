...
from app.repositories.db_support import get_bound_or_latest_user_record, get_profile_by_record_id, today_local
...
class ChatRepository:
    ...
    def get_quota(self) -> dict:
        ...
        session = self._get_or_create_today_session(record.id)
        user_message_count = (
            self.db.scalar(
    ...
    def _get_or_create_today_session(self, record_id: int) -> AiChatSession:
        profile = get_profile_by_record_id(self.db, self.user.id, record_id)
        if profile is None:
            record = self._get_current_record()
            if record is None:
                raise ValueError("record missing")
        ...
        session = self.db.scalar(
            ...
            .where(
                ...
                AiChatSession.profile_id == profile.id,
