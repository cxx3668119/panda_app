<template>
  <main class="page-shell">
    ...
    <div v-if="history.length" class="grid gap-4">
      ...
      >
        <div class="flex items-start justify-between gap-4">
          ...
          <div
            class="font-sans text-4xl font-semibold leading-none text-accent"
          >
            {{ item.score }}
          </div>
        </div>
        ...
        <div class="flex flex-wrap gap-2">
          <span v-for="keyword in item.keywords" :key="keyword" class="chip">{{
            keyword
          }}</span>
        </div>
      </div>
    ...
    <LoadingState v-else text="暂无历史日运" />
    <BottomNav />
  </main>
</template>
