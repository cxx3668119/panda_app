  ...
  <main class="page-shell">
    <section class="grid gap-6 lg:grid-cols-[1.15fr_0.85fr] lg:items-start">
      <div class="page-hero panda-panel rounded-[32px] border border-line/70 bg-panel/75 p-6 pt-6 lg:pt-8">
        ...
        <div class="mt-5 space-y-3">
          ...
          <p class="page-subtitle">
            {{ mode === 'login' ? '使用账号密码登录，进入你的专属主页，管理账号信息、头像、密码与提醒设置。' : '先注册账号，再创建你的专属命盘档案与提醒体验。' }}
          </p>
        </div>
      ...
      <div class="space-y-4">
        ...
        <LoadingState v-else-if="loading" :text="mode === 'login' ? '正在登录...' : '正在注册... '" />
...
async function handleSubmit() {
  ...
  try {
    ...
    error.value = err instanceof Error ? err.message : mode.value === 'login' ? '登录失败，请检查邮箱和密码' : '注册失败，请稍后重试'
  } finally {
    loading.value = false
  }
}
